...
int main() {
    ...
    while (true) {
        ...
        if (get((i - 1 + n) % n, j, k) > get(i, j, k)) {
            i = (i - 1 + n) % n;
            ok = false;
        }
        if (get((i + 1) % n, j, k) > get(i, j, k)) {
            i = (i + 1) % n;
            ok = false;
        }
        if (get(i, (j - 1 + n) % n, k) > get(i, j, k)) {
            j = (j - 1 + n) % n;
            ok = false;
        }
        if (get(i, (j + 1) % n, k) > get(i, j, k)) {
            j = (j + 1) % n;
            ok = false;
        }
        if (get(i, j, (k - 1 + n) % n) > get(i, j, k)) {
            k = (k - 1 + n) % n;
            ok = false;
        }
        if (get(i, j, (k + 1) % n) > get(i, j, k)) {
            k = (k + 1) % n;
            ok = false;
        }
